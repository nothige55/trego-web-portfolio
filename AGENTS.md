...
- 주요 문서:
  - `frontend-strengths.md` — 강조점 우선순위와 채널별 배치 (프론트엔드 전체 대상)
  - `architecture-decisions.md` — 설계 결정과 근거
  - `portfolio-highlights.md` — 기능별 상세와 이식 진행 로그
  - `backend-gaps.md` — 서버 계약의 한계와 공개 배포 전 확인 사항
  - `demo-script.md` — 시연 순서와 GIF 대본
  - `readme-target-draft.md` — 이식 완료 시점의 README 목표 초안
  - `SignalR 이식 현황.md`, `frontend-migration-plan.md`, `testing.md`
...
문서를 작성하거나 강조점·평가를 정리할 때 다음 두 대상을 섞지 않습니다.

| 대상                 | 무엇을 가리키는가                                                                                                                      | 어디에 씁니다                                                                |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| **프론트엔드 전체**  | 원본과 재설계본을 합친 이 제품의 프론트엔드. 계층 트리 설계, 탐색 UX, DnD, 지도 동기화, 실시간 정합성처럼 사용자가 겪는 기능과 그 구현 | `README.md`, 포트폴리오 소개, 면접 답변, vault의 `frontend-strengths.md`     |
| **이식·재설계 작업** | 원본에서 이 저장소로 옮기며 바꾼 것. 진행률, 남은 범위, 원본 대비 before/after, 검증 기록                                              | vault의 `portfolio-highlights.md`·`SignalR 이식 현황.md`, PR 설명, 작업 계획 |

- 이식은 **작업 방식이지 성과가 아닙니다.** `README.md`와 포트폴리오 표면에서 이식 자체를 주제로 삼지 않습니다.
- "원본 대비 개선"은 이식 서술입니다. 근거로 인용할 수는 있어도 주장의 중심에 두지 않습니다.
- 반대로 작업 로그와 이식 현황 문서에서는 이식된 범위와 미이식 항목을 정확히 구분해 적습니다.
- 사용자에게 평가나 분석을 전달할 때도 어느 대상을 말하는지 먼저 밝힙니다. 요청에서 대상이 분명하지 않으면 확인한 뒤 시작합니다.

## 10. 포트폴리오 강조점 기록

기존 Trego 기능을 조사하거나 포트폴리오 저장소로 이식한 뒤에는 사용자에게 결과를 전달하기 전에 다음 문서를 갱신합니다.

`/Users/jinha/trego_docs/portfolio-highlights.md`

- 문서의 중심은 기존 실 프로젝트의 사용자 문제, 실제 기능, 기술적 난이도와 사용자의 기여 후보로 둡니다.
- 마이그레이션 과정, fixture, placeholder, 새 저장소의 구조 정리는 실 프로젝트 기여를 뒷받침하는 보조 증빙으로만 기록합니다.
- 계층형 일정, DnD, 지도, 실시간 협업처럼 기존 코드에서 새롭게 확인한 구현 내용을 기능별 강조점에 누적합니다.
- 기존 실 프로젝트에서 구현된 기능과 포트폴리오 저장소에 현재 이식된 범위를 서로 구분합니다.
- 본인 기여가 불명확한 항목은 확정적으로 표현하지 않고, 최종 포트폴리오 작성 전에 사용자 확인이 필요하다고 남깁니다.
- 테스트 수, 성능, 지원 기능은 현재 코드와 실행 결과로 확인 가능한 값만 기록합니다.
- 기존 프로젝트와 포트폴리오 저장소 모두에서 구현되지 않은 기능을 완료된 것처럼 작성하지 않습니다.
- 이 문서는 개인 장기 기록이므로 저장소 안으로 복사하거나 Git에 커밋하지 않습니다.
