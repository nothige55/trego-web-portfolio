...
- **AI 플래너** (`feat/ai-planner-chat-control`) — 선택한 일정을 문맥으로 AI가 변경을 제안하고 사용자가 작업별로 승인·거절합니다. 승인된 작업은 수동 편집과 같은 커맨드 경로로 실행되어 서버 검증·다른 참여자 전파·되돌리기가 그대로 적용됩니다. 모델 호출은 API 키를 브라우저에 노출하지 않도록 별도 Node Gateway(`ai-gateway/`)가 맡습니다. 기본 동작은 mock이며, 승인 시점의 서버 측 최신 상태 재검증이 남아 있습니다.
...
AI 플래너를 mock이 아닌 실제 모델로 쓰려면 Gateway를 함께 띄우고 `VITE_AI_PLANNER_MODE=api`로 설정합니다. Vite가 `/ai`를 Gateway로 전달합니다.
