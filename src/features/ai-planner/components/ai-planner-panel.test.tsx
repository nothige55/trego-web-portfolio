...
describe("AiPlannerPanel", () => {
  ...
  it("captures the selected Activity as context and returns a reviewable mock proposal", async () => {
    ...
    render(
      <AiPlannerPanel
        contextItems={[{ kind: "activity", name: "성산일출봉", pathId: "activity-seongsan" }]}
      />,
    );
    ...
    await user.click(within(proposal).getByRole("button", { name: "일정에서 미리보기" }));
    expect(within(proposal).getByRole("status")).toHaveTextContent("2건의 미리보기");
  });
