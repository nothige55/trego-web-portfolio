// Shift 클릭 범위를 다중 선택 대상으로 정규화하는 규칙. planner-view-store가 선택을 갱신할 때 호출
...
type SelectionOptions = {
  // Shift 범위는 접힌 자손을 제외한 현재 화면 순서여야 함
...
function getInitialSelectionIds(
  ...
  selectionRange.forEach((item) => {
    ...
    // 양 끝 노드는 사용자가 직접 지정한 작업 대상. 범위 중간의 펼쳐진 부모는
    // 구조를 보여 주는 행일 뿐이므로, 자식이 모두 포함될 때만 아래 단계에서 승격
    if (
...
// getShiftSelectionRange로 별도 반환. 두 결과를 분리해야 부모와 자손이 중복 강조되지 않음
export function calculateShiftSelection({
  ...
  // 화면 순서를 유지하면서 선택된 조상이 대표하는 모든 중복 자손을 제거
