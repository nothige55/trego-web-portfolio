...
// 드래그 중 목록이 "지금 놓으면" 보일 순서다.
// 잡은 노드를 빼고, 투영된 목적지가 화면에 그려지는 자리면 그 앞에 끼운다.
// 목적지가 접힌 컨테이너 안처럼 화면에 없는 곳이면 null을 돌려준다.
export function buildPlannerDragVisualOrder({
  ...
  // 뒤따르는 형제가 화면에 없으면 부모 가지의 끝에 붙인다.
  let at = rest.length;
...
// 그 차이만큼 옮길 거리를 낸다. 레이아웃 높이는 건드리지 않으므로 dnd-kit이 시작 때 잰
// rect가 끝까지 유효하고, 경로 정보 칸이 생기거나 사라지는 것도 이 거리 안에서 흡수된다.
// 보일 순서에 없는 행(목록에서 빠진 잡은 노드)은 결과에 들어가지 않는다.
